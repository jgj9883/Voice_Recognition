...
#engine.say("Sally sells seashells by the searchure.")
#engine.runAndWait()
...
cmdLists = [
        #명령어    대답    종료 리턴값
        [u'끝내자',     '수고하셨습니다',       0],
        [u'종료',       '암호를 맞추면 종료시켜드립니다. 못맞추면 무한반복임 케켘켘',       0],
        [u'암호 땡',       '암호를 맞춰 정상 종료합니다.',       1],
        [u'안녕',       '반가워 난 인공지능이야',        1],
        [u'전규빈',     '잘생겼다',   1],
        [u'학교는',     '동의대학교',      1],
        [u'몇 시냐',     '안알려줌 케켘', 1],
        [u'스마트폰은',  '당연히 아이폰이지',      1],
        [u'일정은',     '세미나 발표 준비해야지 하 인생 쉽지 않다',      1],
        [u'',     '세미나 발표 준비해야지 하 인생 쉽지 않다',      1]
]
...
def CommandProc(stt):
    ...
    for cmdList in cmdLists:
        if str(cmd) == cmdList[0]:
            ...
            os.system('echo %s | espeak -v ko' %cmdList[1])
            # 종료 명령 리턴 0이면 종료
            # 1이면 계속
            return cmdList[2]
        # 명령이 없거
